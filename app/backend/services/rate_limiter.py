...
from services.logging_client import log_narration_outcome, log_query_outcome
...
async def handle_rate_limit_exceeded(request: Request, exc: RateLimitExceeded) -> JSONResponse:
    ...
    if request.url.path == "/api/narrate":
        log_narration_outcome("rate_limited", distinct_id=body.get("distinctId"), guardrail="rate_limit")
    else:
        log_query_outcome(

#!/usr/bin/env python3
"""
PROTOTYPE — e2e_walk_spike_server.py
Fork of e2e_walk_spike.py, kept import-safe for server.py (the working CLI
spike is left untouched — this copy exists so run_pipeline() can be called
in-process from a Flask server without duplicating the whole pipeline a
third time). Only difference from e2e_walk_spike.py: run_pipeline() takes an
open_browser flag (default True, so `python e2e_walk_spike_server.py "..."`
on the CLI still behaves identically) and returns a structured dict instead
of just the ordered species list, so a caller (server.py) can render its own
...
def run_pipeline(user_query, intent_model, description_model, narrative_model, open_browser=True):
    """Returns {"species": [...], "intro": str, "waypoints": [...], "map_path": str}
    on success, or None if no species were found for the query. `open_browser=False`
    is used by server.py, which renders its own view from the returned data rather

...
- **REQ-019**: PostHog's **AI Observability** product is wired in server-side, auto-capturing every real Anthropic call as an `$ai_generation` event (input, output, tokens, cost, latency). **This is gated by the same client-side consent decision as REQ-018**, not fired unconditionally — implemented by having the frontend send its current PostHog `distinct_id` and consent state with each query, and the backend only capturing when consent was granted.
  **Implementation note (2026-08-06):** built using `posthog.ai.anthropic.Anthropic` — a wrapper subclass of the Anthropic client accepting `posthog_distinct_id` per call — instead of the OpenTelemetry `AnthropicInstrumentor()`/`PostHogSpanProcessor` design originally specified here. The OTel approach's `distinct_id` is a `Resource` attribute fixed at process startup, incompatible with REQ-020's genuinely-per-request `distinct_id`. See `docs/decisions/ADR-009-posthog-ai-observability-wrapper-client.md` for the full rationale and alternatives considered.
- **REQ-020**: The frontend's `POST /api/query` request body carries the client's PostHog `distinct_id` — read via `posthog.get_distinct_id()` (`posthog-js`'s own client-generated, client-managed anonymous UUID, already initialized by `lib/posthog.ts`; already an anonymous identifier, no new PII, consistent with the passthrough design already anticipated in `ADR-007`'s Slice 1 implementation note). **No new ID scheme is introduced anywhere in this slice** — backend code only ever passes this value through (into the structured log per REQ-017, and into the PostHog wrapper client's per-call `posthog_distinct_id` per REQ-019 — see `ADR-009`), it never generates or mints an ID itself. This is done so REQ-019's server-side events and the client-side product-analytics events attribute to the same visitor and are joinable in PostHog. **`[NEEDS INPUT during implementation]`: this note is scoped narrowly to dashboard *join verification*, not the ID source above (which is decided) — once REQ-019 is live and both channels are emitting with the same `distinct_id`, confirm in PostHog's actual dashboard whether events sharing a `distinct_id` join automatically in the UI (e.g. on a person's timeline/insight), or whether an explicit server-side `identify()` call is additionally needed. Flagged during design as an assumption to verify against live behavior, not a decision still to make.**
...
- **REQ-024**: A second, distinct test tier — a **pytest-marked eval suite** (e.g. `@pytest.mark.eval`), excluded from the default `pytest` run and from CI's `unit-tests-backend` gate, runnable on demand (`pytest -m eval`) — is built with two levels, both starting with exactly one populated case ("I want to see birds").
  **Implementation note (2026-08-06):** built with plain `pytest`/`assert`, not DeepEval as originally specified below. DeepEval's tracing was verified to give no local console visibility without a paid Confident AI account (contrary to what its docs implied), and no DeepEval metric (e.g. `GEval`) was ever needed — the correctness checks here are deterministic dict comparisons, not something requiring an LLM-judge metric. Dropped entirely; see `docs/decisions/ADR-009-posthog-ai-observability-wrapper-client.md`'s "Alternatives Considered" for detail. Both tiers now also cover adversarial cases (negation, off-topic, purely qualitative, mixed-taxa), not just the one happy-path case below.
  - **Tier 1 (LLM structured-output eval)**: a real Anthropic call, asserting the resolved `taxonFilter` matches expectation (`{"taxonRank": "class", "taxonValue": "Aves"}`) via a plain assertion.
  ...
  - This is explicit foundation-building, not a comprehensive suite — designed to be extended with more cases (and, as query sophistication grows, LLM-as-judge-style metrics for less binary correctness checks, if a framework earns its dependency cost at that point) in later slices (see PRD Slice 12, "Evaluation harness").
...
| `$ai_generation` | Real Anthropic call made | Server-side (posthog.ai.anthropic.Anthropic wrapper client, ADR-009) | Yes (REQ-019) |
...
- `services/ai_observability.py` — `build_client(consent, distinct_id, api_key)` for REQ-019: returns a plain `anthropic.Anthropic` when consent is absent, or the `posthog.ai.anthropic.Anthropic` wrapper client when granted (ADR-009), invoked conditionally per-request based on the consent flag in the request.
...
**New dependencies** (`app/backend/requirements.txt`): `anthropic`, `slowapi`, `google-cloud-secret-manager`, `posthog` (ADR-009 — not `posthog[otel]`/`opentelemetry-sdk`/`opentelemetry-instrumentation-anthropic` as originally listed here). No new dev/test dependency for REQ-024's eval suite — DeepEval was dropped, see REQ-024's implementation note. REQ-022 (PostHog AI Evals) needs no new package — it's configured within the PostHog project itself (a Hog-code rule against captured `$ai_generation` events), not application code.
...
**Testing layout** (`app/backend/tests/`): standard test files for the new deterministic units (REQ-023), plus a distinct eval module/marker configuration for REQ-024 (`tests/evals/`, plain `pytest`/`assert` under a `pytest.ini` `eval` marker, excluded via `addopts = -m "not eval"` from both the default `pytest` run and CI's existing `unit-tests-backend` job). REQ-022 (PostHog AI Evals) has no backend code footprint — it's set up in the PostHog project UI once REQ-019's capture is live, not part of this file layout.
...
- Consent-gating: assert `$ai_generation`/AI Observability capture is skipped when the request's consent flag is absent/false (AC-010) — plain `anthropic.Anthropic` client, no `posthog_distinct_id` passed — and fires when present (AC-011) — `posthog.ai.anthropic.Anthropic` wrapper client used, `posthog_distinct_id` passed through (ADR-009) — mocking `ai_observability.build_client`/the PostHog client at the boundary, not asserting internal call counts beyond whether it was invoked.

**Eval tests** (`@pytest.mark.eval`, real network, excluded from default run/CI — REQ-024):
...
**A pytest-run eval suite for pre-merge regression checks, PostHog AI Evals for live traffic — both kept, at different stages of the same pipeline**: these serve different moments, not the same one. The pytest eval suite (`tests/evals/`, REQ-024) runs entirely offline against deliberately chosen test cases before code ships — the right tool for "does this specific query still resolve correctly." PostHog AI Evals instead runs *after* code ships, scoring real captured `$ai_generation` traffic (from REQ-019's capture) continuously — the right tool for "is the live system still behaving well," including inputs no one thought to write a test case for. **Implementation note (2026-08-06):** the pytest suite was originally planned to be built on DeepEval specifically; it was dropped in favor of plain `pytest`/`assert` once DeepEval's tracing proved to add no functional value here (see ADR-009) — the two-tier reasoning above (pre-merge known cases vs. live-traffic unknown cases) still holds regardless of which framework the pytest tier uses internally.
...
- **PostHog** (EU-hosted, existing project) — the existing client-side product-analytics SDK, the new server-side AI Observability product (`posthog`'s `posthog.ai.anthropic.Anthropic` wrapper client, ADR-009, REQ-019), and PostHog AI Evals (REQ-022, configured within the PostHog project, no new package) as the live-traffic evaluation layer.
...
- Both eval tiers (REQ-024) pass when run manually (`pytest -m eval`) against the real Anthropic and GBIF APIs.

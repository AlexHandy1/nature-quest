...
## Decision
- **Secrets**: GCP Secret Manager is the single source of truth for runtime secrets. Cloud Run pulls secret values directly from Secret Manager at startup; GitHub Actions (via Workload Identity Federation, ADR-005) never sees secret values, only enough access to trigger a deploy. **Note (added during Slice 2):** this policy has not actually been exercised yet — `infra/secret_manager.tf`'s `backend_secret_names` is still `[]`, so no backend runtime secret exists today. The PostHog key turned out not to need this treatment at all: it's a public, frontend build-time value (`VITE_POSTHOG_KEY`, baked into the static bundle via a GitHub Actions *variable*, not a secret, and never touching Secret Manager or Cloud Run's runtime env) — PostHog project tokens are designed to be safely public, similar to a Google Analytics tracking ID. The Anthropic API key (Slice 2) will be the first secret to actually exercise this scaffold, and deliberately uses a different mechanism than the one described above — an explicit Secret Manager client-library fetch in application code, not Cloud Run's native `secret_key_ref` injection — see `docs/specs/spec-tool-llm-guardrails-gbif-query-040826.md` REQ-005/§9 for the rationale (reduces accidental-leak risk specifically relevant to an open-source project with unknown future contributors).

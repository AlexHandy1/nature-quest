...
**Implementation note (2026-08-03):** the health-check endpoint is `/health`, not `/healthz` — `/healthz` is a path reserved by Google's front-end infrastructure on Cloud Run and is intercepted before ever reaching the container (confirmed while building the post-deploy smoke test above). Discovered via a real deploy: `/` and other paths reached the app correctly (visible via `x-cloud-trace-context` in the response headers), but `/healthz` consistently returned a generic Google-branded 404 with no trace header, meaning it never reached Cloud Run's own routing at all.
